...
def simple_bayes(train_letters, test_letters):
    simple_bayes_sequence = 'Simple Bayes: '
    for i in range(0, len(test_letters)):
        test_char = test_letters[i]
        max_prob = 0
        for key in train_letters: # We compare test_char with each character from train_letters.
            hit_count = 1
            miss_count = 1
            space_count = 0
...
test_images = [f'test-{i}-0.png' for i in range(20)]

test_labels = []
with open('test-strings.txt', 'r') as file:
    test_labels = [line.strip() for line in file.readlines()]

i = 0

for test_image in test_images:
    test_letters = load_letters(test_image)
    read_data(str('tra.txt'))

    print(f'\n########## Datapoint number {i}')
    print('The Actual Sentece is:')
    print(test_labels[i])

    print('\n' + simple_bayes(train_letters, test_letters))
    print('\n' + hmm_using_viterbi(train_letters, test_letters))
